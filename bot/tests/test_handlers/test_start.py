...
async def test_yes_cat(monkeypatch):
    # monkeypatch.setattr(cat_bread_bot, "level", {mes['from_id']: 0})
    with patch('bot.handlers.cat_bread_bot.connection.get_level', return_value=0):
    ...
    with patch('bot.handlers.cat_bread_bot.connection.get_level', return_value=1):
